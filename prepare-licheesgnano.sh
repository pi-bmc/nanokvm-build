...
if [ "${SG_BOARD_FAMILY}/${SG_BOARD_LINK}" != "/" ]; then
	...
	if [ $storagetype != sd ]; then
		...
		sed -i 's|${output_dir}/rootfs\.sd|${output_dir}/rootfs.'$storagetype'|g' tools/common/sd_tools/sd_gen_burn_image_rootless.sh
		sed -i 's|rawimages/rootfs\.sd|rawimages/rootfs.'$storagetype'|g' tools/common/sd_tools/sd_gen_burn_image_rootless.sh
		sed -i 's|usb_dl/rootfs\.sd|usb_dl/rootfs.'$storagetype'|g' tools/common/sd_tools/sd_gen_burn_image_rootless.sh
