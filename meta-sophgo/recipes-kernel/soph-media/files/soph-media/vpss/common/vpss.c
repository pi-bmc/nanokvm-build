...
static void vpss_start_handler(void)
{
	...
	for (u8VpssDev = 0; u8VpssDev < VPSS_IP_NUM; u8VpssDev++) {
		 ...
		 * (5.9) is the sanctioned replacement and picks the RT priority
		 * itself, so the driver no longer names one. It returns void -- it
		 * cannot fail for a valid task -- hence no error branch here.
		 */
