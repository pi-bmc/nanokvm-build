...
int vi_create_thread(struct cvi_vi_dev *vdev, enum E_VI_TH th_id)
{
	...
	if (vdev->vi_th[th_id].w_thread == NULL) {
		...
		/*
		 * sched_setscheduler() is not exported to modules; sched_set_fifo()
		 * was added in 5.9 precisely so modules stop open-coding it. It picks
		 * a mid-range FIFO priority rather than the MAX_RT_PRIO-10 this used
		 * to request, which is the intended policy: drivers do not get to
		 * choose their own RT priority any more. These threads only need to
		 * outrank normal tasks, which SCHED_FIFO at any priority does.
		 */
	...
	case AI_ISP_TYPE_BNR:
	{
		...
		case AI_ISP_CFG_INIT:
		{
			...
			if (cfg.param_size == sizeof(ai_isp_bnr_cfg_t)) {
				if (copy_from_user(&bnr_cfg, (void *)(uintptr_t)cfg.param_addr,
						   cfg.param_size))
					return -EFAULT;
