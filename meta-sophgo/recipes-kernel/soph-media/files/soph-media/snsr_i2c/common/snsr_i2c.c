...
/*
 * The vendor tree patched the i2c core with I2C_M_WRSTOP ("allow stop between
 * msg", uapi/linux/i2c.h) and taught its designware driver to emit a STOP
 * after every message of a batched transfer instead of a repeated START.
 * Mainline has no such flag, and I2C_M_STOP needs I2C_FUNC_PROTOCOL_MANGLING
 * which dw-i2c does not advertise.
 *
 * Submitting each message as its own transfer produces exactly the same bus
 * traffic -- every message terminated by a STOP -- using only standard API,
 * so no i2c-core patch is needed. It costs one extra bus turnaround per
 * register write, which is not on any video data path (sensor register
 * programming only).
 */
static int cvi_i2c_transfer_stopped(struct i2c_adapter *adap,
				    struct i2c_msg *msgs, int num)
{
	int i, ret;

	for (i = 0; i < num; i++) {
		ret = i2c_transfer(adap, &msgs[i], 1);
		if (ret != 1)
...
static int snsr_i2c_burst_queue(struct cvi_i2c_dev *dev, struct isp_i2c_data *i2c)
{
	...
	msg->buf = tx;
	msg->len = idx;
	msg->flags = 0;	/* STOP is emitted by cvi_i2c_transfer_stopped() */
...
#endif
		ret = cvi_i2c_transfer_stopped(adap, ctx->msg, ctx->msg_idx);
		if (ret == ctx->msg_idx) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
			ktime_get_real_ts64(&ts);
