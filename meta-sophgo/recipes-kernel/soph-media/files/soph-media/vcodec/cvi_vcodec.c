...
#else
static DEFINE_SEMAPHORE(s_vpu_sem, 1)	/* count arg added in 6.4 */;
#endif
...
{
	/*
	 * mutex_lock_interruptible() is __must_check and this API returns void,
	 * so the vendor code could only ever have ignored a -EINTR and carried
	 * on *without the lock held*. Take the uninterruptible lock: callers
	 * have no way to be told the acquisition failed.
	 */
