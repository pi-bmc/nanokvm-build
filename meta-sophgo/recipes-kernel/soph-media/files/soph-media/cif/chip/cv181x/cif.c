...
#include <generated/utsversion.h>	/* UTS_VERSION moved here in 6.x */
...
/*
 * of_get_named_gpio_flags() was removed upstream and of_get_named_gpio() no
 * longer reports flags, so read the polarity straight out of the phandle's
 * cells. Cell 1 is the standard flags cell for a 2-cell GPIO controller, of
 * which bit 0 is GPIO_ACTIVE_LOW -- the same bit the old enum of_gpio_flags
 * OF_GPIO_ACTIVE_LOW named.
 *
 * The gpiod API would handle polarity for us, but this driver's UAPI
 * (snsr_rst_gpio_s in cif_uapi.h) passes raw GPIO numbers from userspace, so
 * the integer interface has to stay.
 */
static int cif_of_get_gpio_flags(struct device_node *np, const char *propname,
				 int index, int *flags)
{
...
static int _init_resource(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_LINK_NUM; ++i) {
		...
		link->snsr_rst_pin = cif_of_get_gpio_flags(pdev->dev.of_node,
