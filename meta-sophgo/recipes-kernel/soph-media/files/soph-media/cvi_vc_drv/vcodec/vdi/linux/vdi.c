...
/*
 * Was mutex_lock_interruptible(); its __must_check result was discarded, which
 * on a signal meant continuing without the lock. Uninterruptible now, kept as
 * an expression yielding 0 because callers below assign it.
 */
#define MUTEX_LOCK(P_MUTEX_HANDLE) ({ mutex_lock(P_MUTEX_HANDLE); 0; })
