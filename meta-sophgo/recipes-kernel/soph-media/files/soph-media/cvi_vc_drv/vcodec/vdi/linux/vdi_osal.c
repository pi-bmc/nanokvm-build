...
struct cvi_osal_file {
	struct file *filep;
	/*
	 * The vendor kept an mm_segment_t old_fs here for set_fs(KERNEL_DS).
	 * set_fs() was removed in 5.18, and it was never needed: the reads and
	 * writes below go through kernel_read()/kernel_write(), which already
