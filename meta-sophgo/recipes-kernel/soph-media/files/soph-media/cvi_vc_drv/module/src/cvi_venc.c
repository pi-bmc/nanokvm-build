#include "cvi_venc.h"
/*
 * sched_setscheduler() is not exported to modules. sched_set_fifo() (5.9) is
 * the only exported RT setter -- there is no sched_set_rr() -- so the policy
 * moves SCHED_RR -> SCHED_FIFO and the kernel picks the priority.
 *
 * Both are real-time policies, so the intent (outrank normal tasks) holds. The
 * difference only shows up between threads of *equal* priority: RR timeslices
 * among them, FIFO runs one until it blocks. Worth watching if several codec
 * threads are ever runnable at once on this single-core part.
 */
...
#ifndef MIN	/* <linux/minmax.h> defines it since 6.x */
