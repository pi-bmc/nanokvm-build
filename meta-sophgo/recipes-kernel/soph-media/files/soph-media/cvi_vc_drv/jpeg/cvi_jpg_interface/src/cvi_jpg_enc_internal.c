...
int cviJpgEncGetInputDataBuf(CVIJpgHandle jpgHandle, void *data)
{
	...
	/*
	 * Dropped: `if (NULL == pEncInfo->pFrame)`. pFrame is an array
	 * (FRAME_BUF *[MAX_FRAME_JPU]), so the test was on its address and could
	 * never be true -- dead code that GCC 13 now rejects outright. Testing
	 * pFrame[0] instead is probably what was meant, but that would newly
	 * reject sequences the encoder currently accepts, so behaviour is left
	 * as-is pending validation on hardware.
	 */
	if (NULL == data)
		return JPG_RET_INVALID_PARAM;
