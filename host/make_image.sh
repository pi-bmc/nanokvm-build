...
if [ ! -e $bs ]; then
  ...
  cd ${BUILDDIR}/buildroot/dl && git checkout 8688cb2
...
if [ ! -e $bs ]; then
  ...
  cd ${BUILDDIR}/buildroot && git am < /builder/buildroot-overlay-ipmi.patch
...
if [ ! -e $bs ]; then
  ...
  if [ "X${VARIANT}" = "Xkvm" ]; then
    ...
    cd ${BUILDDIR} && ./build-licheervnano.sh --board=${SDK_BOARD_LINK} --maixcdk --maixpy --no-qt5 --panel-variants
  fi
  touch $bs
fi
...
if [ ! -e $bs ]; then
  ...
  cd ${BUILDDIR}/buildroot && rm -rf dl/tpudemo-sg200x/
